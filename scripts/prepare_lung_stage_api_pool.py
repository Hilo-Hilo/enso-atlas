#!/usr/bin/env python3
"""
Build an API-derived LUAD stage training pool from GDC + bucket manifests.

Task:
  - lung stage binary classification
  - label 0: early (Stage I/II)
  - label 1: advanced (Stage III/IV)
...
def iter_tcga_luad_svs() -> Iterable[Dict]:
    filters = {
        "op": "and",
        "content": [
    ...
    while True:
        ...
        if total is None:
            ...
            print(f"GDC LUAD SVS total={total}")
...
def infer_stage_label(case_obj: Dict) -> Tuple[Optional[int], str]:
    """
    Return (label, source):
      0 = early (I/II), 1 = advanced (III/IV), None = unlabeled.
    """
    stages: List[str] = []
    ...
    # Prefer advanced if there are mixed reports.
    for st in stages:
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Prepare API LUAD stage pool from bucket manifests.")
    ...
    ap.add_argument("--out_prefix", type=str, default="trainval_pool_luad_stage_api")
    args = ap.parse_args()
    ...
    for hit in iter_tcga_luad_svs():
        ...
        stage_status = "advanced" if int(lab) == 1 else "early"
    ...
    print(f"Class distribution: {dict(sorted(class_counter.items()))} (0=early,1=advanced)")
