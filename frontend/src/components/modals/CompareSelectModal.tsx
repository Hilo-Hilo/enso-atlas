      ...
      <div className="relative z-10 w-full max-w-2xl mx-4 bg-white rounded-xl shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-gray-50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-gray-900">Compare Cases</h2>
              <p className="text-sm text-gray-500">
                Select a second slide to compare against <span className="font-medium text-gray-700">{slideA.filename}</span>
              </p>
            </div>
          </div>
            ...
            <div className="space-y-2">
              {candidates.map((slide) => (
                <button
                  ...
                  className="w-full text-left p-4 rounded-lg border border-gray-200 hover:border-clinical-400 hover:bg-clinical-50/50 transition-colors"
                >
                  <div className="flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-gray-900 truncate">{slide.filename}</p>
                      <p className="text-xs text-gray-500 font-mono truncate">{slide.id}</p>
                    </div>
                    {slide.label && (
                      <span className="text-xs font-medium text-gray-700 bg-gray-100 border border-gray-200 rounded px-2 py-1 shrink-0">
                        {slide.label}
                      </span>
                    )}
                  </div>
